...
class OptimizationMatchingService {
  ...
  static List<OptimalMatch> findOptimalMatches(
      ...
      for (int j = 0; j < walkRequests.length; j++) {
        ...
        if (owner != null && dog != null) {
          final cost = _calculateOptimizationCost(
            walkers[i], walkRequest, owner, dog, walkers, walkRequests, criteria,
          );
          costMatrix[i][j] = cost;
        }
      }
    ...
    for (int i = 0; i < assignments.length; i++) {
      ...
      if (requestIndex < walkRequests.length && 
        ...
        if (owner != null && dog != null) {
          results.add(OptimalMatch(
            walker: walkers[walkerIndex],
            walkRequest: walkRequest,
            owner: owner,
            dog: dog,
            totalCost: costMatrix[walkerIndex][requestIndex],
            distance: _calculateDistance(walkers[walkerIndex].location!, owner.location!),
            timeConflict: _calculateTimeConflict(walkers[walkerIndex], walkRequest, walkRequests),
          ));
        }
      }
    }
  ...
  static double _calculateOptimizationCost(
    ...
    if (walker.location == null || owner.location == null) return double.infinity;
    
    final distance = _calculateDistance(walker.location!, owner.location!);
    final distanceCost = distance * 0.4;
    final timeConflictCost = _calculateTimeConflict(walker, walkRequest, allRequests) * 0.3;
    final compatibilityCost = _calculateCompatibilityCost(walker, walkRequest, owner, dog) * 0.2;
    final efficiencyCost = _calculateEfficiencyCost(walker, walkRequest, allWalkers, allRequests) * 0.1;
    
    double totalCost = distanceCost + timeConflictCost + compatibilityCost + efficiencyCost;
    ...
    switch (criteria) {
      ...
      case OptimizationCriteria.balanced:
        totalCost = (distanceCost + timeConflictCost + compatibilityCost + efficiencyCost) / 4.0;
        break;
    }
  ...
  static double _calculateTimeConflict(
    ...
    final walkStart = walkRequest.time;
    final walkEnd = walkStart.add(Duration(minutes: walkRequest.duration));
    ...
    for (final otherRequest in allRequests) {
      ...
      final otherStart = otherRequest.time;
      final otherEnd = otherStart.add(Duration(minutes: otherRequest.duration));
  ...
  static double _calculateCompatibilityCost(
    ...
    final walkHour = walkRequest.time.hour;
    String walkTimeSlot;
    ...
    final walkDay = walkRequest.time.weekday % 7;
  ...
  static double _calculateEfficiencyCost(
    ...
    final currentWorkload = allRequests.where((r) => r.walkerId == walker.id).length;
    final maxWorkload = 5;
  ...
  static int _getExperienceLevel(ExperienceLevel level) {
    switch (level) {
      ...
      case ExperienceLevel.expert: return 3;
    }
  }
