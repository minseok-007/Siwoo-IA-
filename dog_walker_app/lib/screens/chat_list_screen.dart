...
class _ChatListScreenState extends State<ChatListScreen> {
    ...
    try {
      ...
      for (final walk in walkRequests) {
        try {
          ...
          if (lastMessage != null || walk.status == WalkRequestStatus.accepted || walk.status == WalkRequestStatus.completed) {
            chats.add({
              'chatId': chatId,
              'walkRequest': walk,
              'otherUser': otherUser,
      ...
      chats.sort((a, b) {
        final aTime = a['lastMessage']?.timestamp ?? a['walkRequest'].time;
        final bTime = b['lastMessage']?.timestamp ?? b['walkRequest'].time;
  ...
  Future<void> _addExistingChats(UserModel user, List<Map<String, dynamic>> chats) async {
    try {
      ...
      for (final chatDoc in chatDocs.docs) {
        ...
        try {
          ...
          if (messagesQuery.docs.isNotEmpty) {
            ...
            final lastMessage = MessageModel.fromFirestore(messagesQuery.docs.first);
            print('Last message in $chatId: ${lastMessage.text} from ${lastMessage.senderId}');
            ...
            if (lastMessage.senderId == user.id || 
              ...
              if (parts.length >= 4 && parts[0] == 'walk') {
                try {
                  ...
                  print('Parsed walk info: walkId=$walkId, ownerId=$ownerId, walkerId=$walkerId');
                        ...
                        title: Row(
                          children: [
                            ...
                            Container(
                              ...
                              child: Text(
                                walkRequest.status.toString().split('.').last.toUpperCase(),
                                style: const TextStyle(
                                  color: Colors.white,
                                  fontSize: 10,
                                  fontWeight: FontWeight.bold,
